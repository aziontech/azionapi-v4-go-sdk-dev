...
func main() {
	applicationId := int32(56) // int32 | A unique integer value identifying the application.
	requestRuleId := int32(56) // int32 | A unique integer value identifying the request rule.
...
**ctx** | **context.Context** | context for authentication, logging, cancellation, deadlines, tracing, etc.
**applicationId** | **int32** | A unique integer value identifying the application. | 
**requestRuleId** | **int32** | A unique integer value identifying the request rule. | 
...
> PaginatedApplicationRequestPhaseRuleEngineList ListApplicationRequestRules(ctx, applicationId).Description(description).Fields(fields).Id(id).IsActive(isActive).Name(name).OrderGte(orderGte).OrderLte(orderLte).Ordering(ordering).Page(page).PageSize(pageSize).Search(search).Execute()
...
func main() {
	...
	orderGte := int32(56) // int32 | Filter by order (greater than or equal to). (optional)
	orderLte := int32(56) // int32 | Filter by order (less than or equal to). (optional)
	...
	configuration := openapiclient.NewConfiguration()
	apiClient := openapiclient.NewAPIClient(configuration)
	resp, r, err := apiClient.ApplicationsRequestRulesAPI.ListApplicationRequestRules(context.Background(), applicationId).Description(description).Fields(fields).Id(id).IsActive(isActive).Name(name).OrderGte(orderGte).OrderLte(orderLte).Ordering(ordering).Page(page).PageSize(pageSize).Search(search).Execute()
	if err != nil {
 ...
 **orderGte** | **int32** | Filter by order (greater than or equal to). | 
 **orderLte** | **int32** | Filter by order (less than or equal to). | 
