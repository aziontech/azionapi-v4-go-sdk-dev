...
> PaginatedUserList ListUsers(ctx).Fields(fields).Locked(locked).Ordering(ordering).Page(page).PageSize(pageSize).Search(search).Execute()
...
func main() {
	...
	// response from `ListUsers`: PaginatedUserList
