# Go API client for storageapi/api/openapi
...
```go
import storageapi/api/openapi "github.com/GIT_USER_ID/GIT_REPO_ID"
```
...
For using other server than the one defined on index 0 set context value `storageapi/api/openapi.ContextServerIndex` of type `int`.

```go
ctx := context.WithValue(context.Background(), storageapi/api/openapi.ContextServerIndex, 1)
```

### Templated Server URL

Templated server URL is formatted using default variables from configuration or from context value `storageapi/api/openapi.ContextServerVariables` of type `map[string]string`.

```go
ctx := context.WithValue(context.Background(), storageapi/api/openapi.ContextServerVariables, map[string]string{
	"basePath": "v2",
})
```
...
Similar rules for overriding default operation server index and variables applies by using `storageapi/api/openapi.ContextOperationServerIndices` and `storageapi/api/openapi.ContextOperationServerVariables` context maps.

```go
ctx := context.WithValue(context.Background(), storageapi/api/openapi.ContextOperationServerIndices, map[string]int{
	"{classname}Service.{nickname}": 2,
})
ctx = context.WithValue(context.Background(), storageapi/api/openapi.ContextOperationServerVariables, map[string]map[string]string{
...
```go
auth := context.WithValue(context.Background(), storageapi/api/openapi.ContextAccessToken, "BEARER_TOKEN_STRING")
...
auth := context.WithValue(
		context.Background(),
		storageapi/api/openapi.ContextAPIKeys,
		map[string]storageapi/api/openapi.APIKey{
