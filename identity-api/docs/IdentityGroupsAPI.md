...
> PaginatedResponseListGroupList ListGroup(ctx).Fields(fields).Ordering(ordering).Page(page).PageSize(pageSize).Search(search).Execute()
...
func main() {
	...
	// response from `ListGroup`: PaginatedResponseListGroupList
...
[**PaginatedResponseListGroupList**](PaginatedResponseListGroupList.md)
