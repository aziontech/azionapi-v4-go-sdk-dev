...
> PaginatedResponseListGrantList ListGrants(ctx).Fields(fields).Ordering(ordering).Page(page).PageSize(pageSize).Search(search).Execute()
...
func main() {
	...
	// response from `ListGrants`: PaginatedResponseListGrantList
...
[**PaginatedResponseListGrantList**](PaginatedResponseListGrantList.md)
