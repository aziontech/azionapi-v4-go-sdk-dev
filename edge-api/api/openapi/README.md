# Go API client for edgeapi/api/openapi
...
```go
import edgeapi/api/openapi "github.com/GIT_USER_ID/GIT_REPO_ID"
```
...
For using other server than the one defined on index 0 set context value `edgeapi/api/openapi.ContextServerIndex` of type `int`.

```go
ctx := context.WithValue(context.Background(), edgeapi/api/openapi.ContextServerIndex, 1)
```

### Templated Server URL

Templated server URL is formatted using default variables from configuration or from context value `edgeapi/api/openapi.ContextServerVariables` of type `map[string]string`.

```go
ctx := context.WithValue(context.Background(), edgeapi/api/openapi.ContextServerVariables, map[string]string{
	"basePath": "v2",
})
```
...
Similar rules for overriding default operation server index and variables applies by using `edgeapi/api/openapi.ContextOperationServerIndices` and `edgeapi/api/openapi.ContextOperationServerVariables` context maps.

```go
ctx := context.WithValue(context.Background(), edgeapi/api/openapi.ContextOperationServerIndices, map[string]int{
	"{classname}Service.{nickname}": 2,
})
ctx = context.WithValue(context.Background(), edgeapi/api/openapi.ContextOperationServerVariables, map[string]map[string]string{
...
## Documentation For Models
 ...
 - [ApplicationRuleEngineStringAttributesValue1](docs/ApplicationRuleEngineStringAttributesValue1.md)
...
```go
auth := context.WithValue(context.Background(), edgeapi/api/openapi.ContextAccessToken, "BEARER_TOKEN_STRING")
...
auth := context.WithValue(
		context.Background(),
		edgeapi/api/openapi.ContextAPIKeys,
		map[string]edgeapi/api/openapi.APIKey{
