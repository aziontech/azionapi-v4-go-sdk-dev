/*
edge-api

REST API OpenAPI documentation for the edge-api

API version: 1.0.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package edge-api

import (
	"encoding/json"
	"bytes"
	"fmt"
)

// checks if the WorkloadDeploymentBindsRequest type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &WorkloadDeploymentBindsRequest{}

// WorkloadDeploymentBindsRequest struct for WorkloadDeploymentBindsRequest
type WorkloadDeploymentBindsRequest struct {
	EdgeApplication int64 `json:"edge_application"`
	EdgeFirewall NullableInt64 `json:"edge_firewall,omitempty"`
	CustomPage NullableInt64 `json:"custom_page,omitempty"`
}

type _WorkloadDeploymentBindsRequest WorkloadDeploymentBindsRequest

// NewWorkloadDeploymentBindsRequest instantiates a new WorkloadDeploymentBindsRequest object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewWorkloadDeploymentBindsRequest(edgeApplication int64) *WorkloadDeploymentBindsRequest {
	this := WorkloadDeploymentBindsRequest{}
	this.EdgeApplication = edgeApplication
	return &this
}

// NewWorkloadDeploymentBindsRequestWithDefaults instantiates a new WorkloadDeploymentBindsRequest object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewWorkloadDeploymentBindsRequestWithDefaults() *WorkloadDeploymentBindsRequest {
	this := WorkloadDeploymentBindsRequest{}
	return &this
}

// GetEdgeApplication returns the EdgeApplication field value
func (o *WorkloadDeploymentBindsRequest) GetEdgeApplication() int64 {
	if o == nil {
		var ret int64
		return ret
	}

	return o.EdgeApplication
}

// GetEdgeApplicationOk returns a tuple with the EdgeApplication field value
// and a boolean to check if the value has been set.
func (o *WorkloadDeploymentBindsRequest) GetEdgeApplicationOk() (*int64, bool) {
	if o == nil {
		return nil, false
	}
	return &o.EdgeApplication, true
}

// SetEdgeApplication sets field value
func (o *WorkloadDeploymentBindsRequest) SetEdgeApplication(v int64) {
	o.EdgeApplication = v
}

// GetEdgeFirewall returns the EdgeFirewall field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *WorkloadDeploymentBindsRequest) GetEdgeFirewall() int64 {
	if o == nil || IsNil(o.EdgeFirewall.Get()) {
		var ret int64
		return ret
	}
	return *o.EdgeFirewall.Get()
}

// GetEdgeFirewallOk returns a tuple with the EdgeFirewall field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *WorkloadDeploymentBindsRequest) GetEdgeFirewallOk() (*int64, bool) {
	if o == nil {
		return nil, false
	}
	return o.EdgeFirewall.Get(), o.EdgeFirewall.IsSet()
}

// HasEdgeFirewall returns a boolean if a field has been set.
func (o *WorkloadDeploymentBindsRequest) HasEdgeFirewall() bool {
	if o != nil && o.EdgeFirewall.IsSet() {
		return true
	}

	return false
}

// SetEdgeFirewall gets a reference to the given NullableInt64 and assigns it to the EdgeFirewall field.
func (o *WorkloadDeploymentBindsRequest) SetEdgeFirewall(v int64) {
	o.EdgeFirewall.Set(&v)
}
// SetEdgeFirewallNil sets the value for EdgeFirewall to be an explicit nil
func (o *WorkloadDeploymentBindsRequest) SetEdgeFirewallNil() {
	o.EdgeFirewall.Set(nil)
}

// UnsetEdgeFirewall ensures that no value is present for EdgeFirewall, not even an explicit nil
func (o *WorkloadDeploymentBindsRequest) UnsetEdgeFirewall() {
	o.EdgeFirewall.Unset()
}

// GetCustomPage returns the CustomPage field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *WorkloadDeploymentBindsRequest) GetCustomPage() int64 {
	if o == nil || IsNil(o.CustomPage.Get()) {
		var ret int64
		return ret
	}
	return *o.CustomPage.Get()
}

// GetCustomPageOk returns a tuple with the CustomPage field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *WorkloadDeploymentBindsRequest) GetCustomPageOk() (*int64, bool) {
	if o == nil {
		return nil, false
	}
	return o.CustomPage.Get(), o.CustomPage.IsSet()
}

// HasCustomPage returns a boolean if a field has been set.
func (o *WorkloadDeploymentBindsRequest) HasCustomPage() bool {
	if o != nil && o.CustomPage.IsSet() {
		return true
	}

	return false
}

// SetCustomPage gets a reference to the given NullableInt64 and assigns it to the CustomPage field.
func (o *WorkloadDeploymentBindsRequest) SetCustomPage(v int64) {
	o.CustomPage.Set(&v)
}
// SetCustomPageNil sets the value for CustomPage to be an explicit nil
func (o *WorkloadDeploymentBindsRequest) SetCustomPageNil() {
	o.CustomPage.Set(nil)
}

// UnsetCustomPage ensures that no value is present for CustomPage, not even an explicit nil
func (o *WorkloadDeploymentBindsRequest) UnsetCustomPage() {
	o.CustomPage.Unset()
}

func (o WorkloadDeploymentBindsRequest) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o WorkloadDeploymentBindsRequest) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["edge_application"] = o.EdgeApplication
	if o.EdgeFirewall.IsSet() {
		toSerialize["edge_firewall"] = o.EdgeFirewall.Get()
	}
	if o.CustomPage.IsSet() {
		toSerialize["custom_page"] = o.CustomPage.Get()
	}
	return toSerialize, nil
}

func (o *WorkloadDeploymentBindsRequest) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"edge_application",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err;
	}

	for _, requiredProperty := range(requiredProperties) {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varWorkloadDeploymentBindsRequest := _WorkloadDeploymentBindsRequest{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	err = decoder.Decode(&varWorkloadDeploymentBindsRequest)

	if err != nil {
		return err
	}

	*o = WorkloadDeploymentBindsRequest(varWorkloadDeploymentBindsRequest)

	return err
}

type NullableWorkloadDeploymentBindsRequest struct {
	value *WorkloadDeploymentBindsRequest
	isSet bool
}

func (v NullableWorkloadDeploymentBindsRequest) Get() *WorkloadDeploymentBindsRequest {
	return v.value
}

func (v *NullableWorkloadDeploymentBindsRequest) Set(val *WorkloadDeploymentBindsRequest) {
	v.value = val
	v.isSet = true
}

func (v NullableWorkloadDeploymentBindsRequest) IsSet() bool {
	return v.isSet
}

func (v *NullableWorkloadDeploymentBindsRequest) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableWorkloadDeploymentBindsRequest(val *WorkloadDeploymentBindsRequest) *NullableWorkloadDeploymentBindsRequest {
	return &NullableWorkloadDeploymentBindsRequest{value: val, isSet: true}
}

func (v NullableWorkloadDeploymentBindsRequest) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableWorkloadDeploymentBindsRequest) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


