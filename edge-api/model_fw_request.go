/*
edge-api

REST API OpenAPI documentation for the edge-api

API version: 1.0.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package edgeapi

import (
	"encoding/json"
	"bytes"
	"fmt"
)

// checks if the FwRequest type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &FwRequest{}

// FwRequest struct for FwRequest
type FwRequest struct {
	Name string `json:"name"`
	Modules *FwModulesRequest `json:"modules,omitempty"`
	Debug *bool `json:"debug,omitempty"`
	Active *bool `json:"active,omitempty"`
}

type _FwRequest FwRequest

// NewFwRequest instantiates a new FwRequest object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewFwRequest(name string) *FwRequest {
	this := FwRequest{}
	this.Name = name
	return &this
}

// NewFwRequestWithDefaults instantiates a new FwRequest object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewFwRequestWithDefaults() *FwRequest {
	this := FwRequest{}
	return &this
}

// GetName returns the Name field value
func (o *FwRequest) GetName() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Name
}

// GetNameOk returns a tuple with the Name field value
// and a boolean to check if the value has been set.
func (o *FwRequest) GetNameOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Name, true
}

// SetName sets field value
func (o *FwRequest) SetName(v string) {
	o.Name = v
}

// GetModules returns the Modules field value if set, zero value otherwise.
func (o *FwRequest) GetModules() FwModulesRequest {
	if o == nil || IsNil(o.Modules) {
		var ret FwModulesRequest
		return ret
	}
	return *o.Modules
}

// GetModulesOk returns a tuple with the Modules field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *FwRequest) GetModulesOk() (*FwModulesRequest, bool) {
	if o == nil || IsNil(o.Modules) {
		return nil, false
	}
	return o.Modules, true
}

// HasModules returns a boolean if a field has been set.
func (o *FwRequest) HasModules() bool {
	if o != nil && !IsNil(o.Modules) {
		return true
	}

	return false
}

// SetModules gets a reference to the given FwModulesRequest and assigns it to the Modules field.
func (o *FwRequest) SetModules(v FwModulesRequest) {
	o.Modules = &v
}

// GetDebug returns the Debug field value if set, zero value otherwise.
func (o *FwRequest) GetDebug() bool {
	if o == nil || IsNil(o.Debug) {
		var ret bool
		return ret
	}
	return *o.Debug
}

// GetDebugOk returns a tuple with the Debug field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *FwRequest) GetDebugOk() (*bool, bool) {
	if o == nil || IsNil(o.Debug) {
		return nil, false
	}
	return o.Debug, true
}

// HasDebug returns a boolean if a field has been set.
func (o *FwRequest) HasDebug() bool {
	if o != nil && !IsNil(o.Debug) {
		return true
	}

	return false
}

// SetDebug gets a reference to the given bool and assigns it to the Debug field.
func (o *FwRequest) SetDebug(v bool) {
	o.Debug = &v
}

// GetActive returns the Active field value if set, zero value otherwise.
func (o *FwRequest) GetActive() bool {
	if o == nil || IsNil(o.Active) {
		var ret bool
		return ret
	}
	return *o.Active
}

// GetActiveOk returns a tuple with the Active field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *FwRequest) GetActiveOk() (*bool, bool) {
	if o == nil || IsNil(o.Active) {
		return nil, false
	}
	return o.Active, true
}

// HasActive returns a boolean if a field has been set.
func (o *FwRequest) HasActive() bool {
	if o != nil && !IsNil(o.Active) {
		return true
	}

	return false
}

// SetActive gets a reference to the given bool and assigns it to the Active field.
func (o *FwRequest) SetActive(v bool) {
	o.Active = &v
}

func (o FwRequest) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o FwRequest) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["name"] = o.Name
	if !IsNil(o.Modules) {
		toSerialize["modules"] = o.Modules
	}
	if !IsNil(o.Debug) {
		toSerialize["debug"] = o.Debug
	}
	if !IsNil(o.Active) {
		toSerialize["active"] = o.Active
	}
	return toSerialize, nil
}

func (o *FwRequest) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"name",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err;
	}

	for _, requiredProperty := range(requiredProperties) {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varFwRequest := _FwRequest{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	err = decoder.Decode(&varFwRequest)

	if err != nil {
		return err
	}

	*o = FwRequest(varFwRequest)

	return err
}

type NullableFwRequest struct {
	value *FwRequest
	isSet bool
}

func (v NullableFwRequest) Get() *FwRequest {
	return v.value
}

func (v *NullableFwRequest) Set(val *FwRequest) {
	v.value = val
	v.isSet = true
}

func (v NullableFwRequest) IsSet() bool {
	return v.isSet
}

func (v *NullableFwRequest) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableFwRequest(val *FwRequest) *NullableFwRequest {
	return &NullableFwRequest{value: val, isSet: true}
}

func (v NullableFwRequest) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableFwRequest) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


